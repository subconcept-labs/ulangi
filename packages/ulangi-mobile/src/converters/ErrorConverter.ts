...
export class ErrorConverter {
  public convertToMessage(errorBag: ErrorBag): string {
    ...
    switch (errorBag.errorCode) {
      ...
      case ErrorCode.IAP__PURCHASES_ALREADY_APPLIED_TO_OTHER_ACCOUNTS:
        const account = Platform.select({
          ios: 'Apple ID',
          android: 'Google account',
        });

        message = `You have used this ${account} to make purchases for other Ulangi accounts. Those purchases cannot be reapplied to this account. To make purchases or upgrade, you must use another ${account}. Sorry for the inconvenience.`;
        break;
