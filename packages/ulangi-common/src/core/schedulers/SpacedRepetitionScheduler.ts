...
export class SpacedRepetitionScheduler extends Scheduler {
  ...
  public getNextLevel(
    ...
    return this.calculateNextLevel(vocabulary.level, feedback, maxLevel);
  }
  ...
  public getNetLevelChange(
    ...
    return this.calculateNetLevelChange(vocabulary.level, feedback, maxLevel);
  }
  ...
  public getReviewTimeFromNow(
    ...
    const currentLevel = vocabulary.level;
    if (currentLevel >= maxLevel) {
      return 'done';
    } else if (vocabulary.lastLearnedAt === null) {
      return 'now';
    } else {
      return this.calculateReviewTimeFromNow(
        initialInterval,
        currentLevel,
        moment(vocabulary.lastLearnedAt).toDate(),
        shortForm,
        withoutSuffix
      );
    }
  ...
  public willBeArchived(
    ...
    return this.archiveDecider.shouldArchive(
      {
        spacedRepetitionLevel: this.getNextLevel(
          vocabulary,
          feedback,
          maxLevel
        ),
