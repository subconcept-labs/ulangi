...
import { WritingScheduler } from './WritingScheduler';

describe('WritingScheduler Tests', (): void => {
  const maxLevel = 10;
  let writingScheduler: WritingScheduler;

  beforeEach(
    (): void => {
      writingScheduler = new WritingScheduler();
    }
  );
  ...
  describe('getNextLevel', (): void => {
    it(`return next level for POOR correctly`, (): void => {
      ...
      expectedResults.forEach(
        (result): void => {
          expect(
            writingScheduler.getNextLevel(
              new VocabularyBuilder().build({
                writing: { level: result.currentLevel },
              }),
              Feedback.POOR,
              maxLevel
            )
  ...
  describe('getNetLevelChange', (): void => {
    it(`return net level change for POOR correctly`, (): void => {
      ...
      expectedResults.forEach(
        (result): void => {
          expect(
            writingScheduler.getNetLevelChange(
              new VocabularyBuilder().build({
                writing: { level: result.currentLevel },
              }),
              Feedback.POOR,
  ...
  describe('getReviewTimeFromNow', (): void => {
    it(`return review time from now correctly`, (): void => {
      ...
      expectedResults.forEach(
        (result): void => {
          expect(
            writingScheduler.getReviewTimeFromNow(
              12,
              new VocabularyBuilder().build({
                writing: {
  ...
  describe('getNextReviewTimeFromNow', (): void => {
    it(`return next review time from now correctly`, (): void => {
      ...
      expectedResults.forEach(
        (result): void => {
          expect(
            writingScheduler.getNextReviewTimeFromNow(
              12,
              new VocabularyBuilder().build({
                writing: {
                  level: result.currentLevel,
  ...
  describe('willBeArchived', (): void => {
    it(`return true if archive conditions are met`, (): void => {
      ...
      expect(
        writingScheduler.willBeArchived(
          new VocabularyBuilder().build({
            level: 5,
            writing: {
              level: 4,
